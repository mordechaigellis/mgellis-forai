...
create table dbo.Recipe(
    ...
    DateDrafted datetime not null default getdate()
        constraint ck_Recipe_date_drafted_must_be_before_today check(DateDrafted <= getdate()),
...
create table dbo.RecipeIngredient(
    ...
    constraint u_RecipeIngredient_RecipeId_SequenceVal unique(RecipeId,SequenceVal)
)
go
...
create table dbo.Meal(
    ...
    DateCreated date not null default getdate() constraint ck_Meal_DateCreated_must_be_before_today check(DateCreated <= getdate()),
...
create table dbo.Cookbook(
    ...
    Active bit not null default 1,
    DateCreated date not null default getdate() constraint ck_Cookbook_DateCreated_must_be_before_today check(DateCreated <= getdate()),
